...
*/
	...
	Rectangle c_bounds(0, 0, GetClientAreaBounds().width, 500);
...

		   {
		      if (__pCanvas)
		      {
		    	  ...
		    	  __pCanvas->DrawText(FloatPoint(20, (float)200/650*500), String(L"Battery state"));
		    	  ...
		    	  __pCanvas->DrawText(FloatPoint(20, (float)460/650*500), String(L"LCD state"));
		    	  __pCanvas->DrawText(FloatPoint(20, (float)540/650*500), String(L"Wifi state"));

		    	  Font font2;
		    	  font2.Construct(FONT_STYLE_PLAIN, 10);
		    	  __pCanvas->SetFont(font2);
		    	  __pCanvas->DrawText(FloatPoint(10, (float)220/650*500), String(L"100%"));
		    	  __pCanvas->DrawText(FloatPoint(10, (float)440/650*500), String(L"0%"));

		    	  __pCanvas->SetForegroundColor(Color::GetColor(COLOR_ID_GREEN));
		    	  __pCanvas->SetLineWidth(4);

		    	  for(int i = 1; i < bic; i++) {
		    		  __pCanvas->DrawLine(FloatPoint(20 + (float)bi[i-1][0]/max*(GetClientAreaBounds().width-40), (float)450/650*500 - (float)bi[i-1][1]/100*220/650*500),
		    				  FloatPoint(20 + (float)bi[i][0]/max*(GetClientAreaBounds().width-40), (float)450/650*500 - (float)bi[i][1]/100*220/650*500));
		    	  }
		    	  if(bic >=1 && bic < max)
		    		  __pCanvas->DrawLine(FloatPoint(20 + (float)bi[bic-1][0]/max*(GetClientAreaBounds().width-40), (float)450/650*500 - (float)bi[bic-1][1]/100*220/650*500),
		    		  		    				  FloatPoint(20 + (float)(GetClientAreaBounds().width-40), (float)450/650*500 - (float)bi[bic-1][1]/100*220/650*500));
		    	  ...
		    	  for(int i = 1; i < lic; i++) {
		    		  	  if(li[i-1][1] != 0)
		    		  		 //AppLog("ESLAB %f %f", 20 + (float)li[i-1][0]/max*(GetClientAreaBounds().width-40), ((float)li[i][0] - li[i-1][0])/max*(GetClientAreaBounds().width-40));
		    		  		  __pCanvas->FillRectangle(Color(40, 120, 182, 0xFF),
		    		  				  FloatRectangle(20 + (float)li[i-1][0]/max*(GetClientAreaBounds().width-40), (float)490/650*500, ((float)li[i][0] - li[i-1][0])/max*(GetClientAreaBounds().width-40), (float)50/650*500));
	  		      }


		    	  if(lic >=1 && lic < max && li[lic-1][1] != 0)
		    		  __pCanvas->FillRectangle(Color(40, 120, 182, 0xFF),
		    				  FloatRectangle(20 + (float)li[lic-1][0]/max*(GetClientAreaBounds().width-40), (float)490/650*500, ((float)max - li[lic-1][0])/max*(GetClientAreaBounds().width-40), (float)50/650*500));
		    	  ...
		    	  for(int i = 1; i < wic; i++) {
		    	  		    		  	  if(wi[i-1][1] != 0)
		    	  		    		  		  __pCanvas->FillRectangle(Color(40, 120, 182, 0xFF),
		    	  		    		  				  FloatRectangle(20 + (float)wi[i-1][0]/max*(GetClientAreaBounds().width-40), (float)570/650*500, ((float)wi[i][0] - wi[i-1][0])/max*(GetClientAreaBounds().width-40), (float)50/650*500));
		    	  }
		    	  if(wic >=1 && wic < max && wi[wic-1][1] != 0)
		    	  		    		  __pCanvas->FillRectangle(Color(40, 120, 182, 0xFF),
		    	  		    FloatRectangle(20 + (float)wi[wic-1][0]/max*(GetClientAreaBounds().width-40), (float)570/650*500, ((float)max - wi[wic-1][0])/max*(GetClientAreaBounds().width-40), (float)50/650*500));
